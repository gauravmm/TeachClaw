...
Allows Claude Code sessions to connect to teachclaw for full bidirectional
...
teachclaw, and teachclaw's responses arrive as <channel> tags in its context.
"""
...
from teachclaw.bus import MessageBus, OutboundMessage
from teachclaw.channels.attention import AttentionPolicy
...
class ClaudeCodeConfig(ChannelConfig):
    ...
    host: str = "127.0.0.1"
    port: int = 18791
    channel_name: str = "teachclaw"
    instructions: str = (
        "You are connected to teachclaw via the Claude channels protocol. "
        "Call the reply tool to send a message to teachclaw. "
        "Messages from teachclaw arrive as <channel> tags in your context."
    )
...
class ClaudeCodeChannel(BaseChannel):
    ...
    def _build_mcp_server(self) -> Server:
        ...
        async def list_tools() -> list[types.Tool]:
            return [
                types.Tool(
                    name="reply",
                    description=(
                        "Send a message to teachclaw. "
                        "Call this when you receive a <channel> tag and want to respond, "
                        "or to initiate contact with teachclaw's agent."
                    ),
                    inputSchema={
                        "type": "object",
                        "properties": {
                            "content": {
                                "type": "string",
                                "description": "The message text to send to teachclaw.",
                            },
                            "sender_id": {
                                "type": "string",
                                "description": (
        ...
        async def call_tool(name: str, arguments: dict[str, Any]) -> list[types.TextContent]:
            ...
            return [types.TextContent(type="text", text="Delivered to teachclaw.")]
